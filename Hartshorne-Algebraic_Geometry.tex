...
\begin{proof}
  ...
  $\closure{Y} \subseteq Y_1 \cup Y_2$ where $Y_1$ and $Y_2$ are closed in $X$ and
  $\closure{Y} \nsubseteq Y_1$. Then $Y \subseteq Y_1 \cup Y_2$ and $Y \nsubseteq
  Y_1$ (otherwise $\closure{Y} \subseteq Y_1$), which together with $Y$ being
  irreducible implies $Y \subseteq Y_2$, thus $\closure{Y} \subseteq Y_2$.

  Conversely, let $\closure{Y}$ be irreducible, we show $Y$ is also irreducible.
  Assume $Y \subseteq Y_1 \cup Y_2$ where $Y_1$ and $Y_2$ are closed in $X$ and $Y
  \nsubseteq Y_1$. Then $\closure{Y} \subseteq Y_1 \cup Y_2$ (because $Y_1 \cup Y_2$
  is closed) and obviously $\closure{Y} \nsubseteq Y_1$, so $\closure{Y} \subseteq
  Y_2$, implying $Y \subseteq Y_2$.
\end{proof}
...
\begin{proof}
  ...
  \polynomials{k}{x_1}{\dots}{x_n}}{f(P) = 0\text{ for all }P \in Y}$. Now if $f^r
  \in I(Y)$, then for all $P \in Y$, $f^r(P) = 0$, thus $f(P) = 0$.

  Conversely, suppose $I(Y)$ is prime. Using Proposition 1.2.(e) again, we have
  $\closure{Y} = Z(I(Y))$. It then follows from Corollary 1.4 that $\closure{Y}$ is
  irreducible, and so is $Y$ by the proposition above.
\end{proof}
...
\Ex
For one example, $f = x^2 + y^2 + 1$ is irreducible in $\polynomials{\reals}{x}{y}$
(can be proved by assuming factors of $f$, viewing them as polynomials in
$\polynomials{\polynomials{\reals}{x}}{y}$), but $Z(f)$ in $\Affine[\reals]{2}$ is
$\emptyset$ and not considered to be irreducible by definition of irreducible sets.

For another example, $f = x^2(x-1)^2 + y^2$ is irreducible in
$\polynomials{\reals}{x}{y}$ (the unique factorization of $f$ in
$\polynomials{\complexes}{x}{y}$ is $(x(x-1) + iy)(x(x-1) - iy)$, but none of the
two factors is in $\polynomials{\reals}{x}{y}$), but $Z(f) = \set{(0, 0)}{(1, 0)}$
is not irreducible.
...
\begin{enumerate}
  ...
  \item One direction is easy. To show $\rho_d^{-1}$ is continuous, one needs to
        look at the open sets $U_i$.
\end{enumerate}
...
$\rational{V}{\psi_V}$ be dominant from $Y$ to $Z$. The composition is defined where
$\psi_V\circ\varphi_U$ is defined. Let $V'\coloneqq\varphi_U(U)\cap V$. If
$W\coloneqq\closure{\psi_V(V')}$ is not $Z$, then obviously $\psi_V^{-1}(W)
\subsetneq V$, thus $\psi_V^{-1}(Z\setminus W)$ is a nonempty open subset of $V$,
thus open in $Y$. Now $\varphi_U(U) \cap \psi_V^{-1}(Z\setminus W) = \varnothing$
(since $\varphi_U(U) \cap V \subset V' \subset \psi_V^{-1}(W)$), contradicting that
$\varphi_U(U)$ is dense.
...
\begin{enumerate}
  \item If $Y$ were to be singular at some point, then it must be that $y = 0$ and
        $3x^2-1 = 0$, which contradicts $y^2 = x^3 - x$. Since $Y$ is nonsingular
        every localization of $A$ is integrally closed, so $A$ is integrally closed.
  \item Only need to show that if $f(x) \in \polynomials{k}{x}$ vanishes on $Y$,
        then $f(x)$ is the zero polynomial. For every $x\in k$ there is some $P$
        whose first coordinate is $x$, thus $f(x) = 0$ for every $x$ and we are
        done. $A$ is in the integral closure of $\polynomials{k}{x}$ since $y^2 +
        x^3 - x = 0$ in $K$. Then using the fact that $A$ is integrally closed we
        get the result. \item
\end{enumerate}
\end{document}
