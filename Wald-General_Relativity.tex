...
\Ex \textbf{Failed attempt.} For any $p\in M$, we define a map $\phi$ from an
open ball $B$ of $\reals^n$ centered at $0$ to $M$ as follows. First of all let
$\phi(0) = p$. For any $x = \prn{x^1, \dots, x^n}\in B$, let $C\colon [0, 1]
    \to B$ be a smooth curve such that $C(0) = 0$ and $C(1) = x$. We require that
$\phi$ satisfies the equation
\begin{equation*}
    \dv{(\phi\circ C)}{t}(s) = \sum_\mu \dv{C^\mu}{t}(s) \vat{Y_\mu}{\phi\circ
        C(s)}, \quad \text{for all } s \in [0,1].
\end{equation*}
If we pick a coordinate system in a neighborhood of $p$, the problem of solving
the equation reduces to solving the system,
\begin{equation*}
    \dv{(\phi\circ C)^\nu}{t}(s) = \sum_\mu \dv{C^\mu}{t}(s)
    {Y_\mu}^\nu\prn{\phi\circ C(s)}, \quad \text{for } \nu = 1, \cdots, n.
\end{equation*}
\textit{At this point I found it hard to proceed so I looked at hints. Below we
    start from scratch using hints.}

\textbf{2nd attempt.} Essentially we are trying to solve for the equations
\begin{equation*}
    \pdv{f^\nu}{y^\mu} = {Y_\mu}^\nu, \quad \mu = 1, \cdots, n,
\end{equation*}
for functions $f^\nu, \nu = 1, \cdots, n$ on an open ball of $\reals^n$ and
${Y_\mu}^\nu$ are the components of $Y_\mu$ in a coordinate basis.

\textbf{3rd attempt.} Instead of constructing the map whose partial derivatives
directly lead to ${Y_\mu}$ (as in 2nd attempt), we try to construct the inverse
map first. Observe that the equations
\begin{equation*}
    \pdv{f^\nu}{x^\mu} = ({Y^\nu}^*)_\mu, \quad \mu = 1, \cdots, n,
\end{equation*}
for functions $f^\nu, \nu = 1, \cdots, n$ have solutions due to 4(b).
